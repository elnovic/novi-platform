...
import { getCours, getUtilisateurConnecte } from '@/lib/supabase'
...
export default function Certificat({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    getCours(id).then(setCours)
    getUtilisateurConnecte().then(setApprenant)
  }, [id])
  ...
  const telechargerPDF = async () => {
    ...
    const canvas = await html2canvas(certificatRef.current, { scale: 2, backgroundColor: '#ffffff' })
  ...
  if (!cours) return <main className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin" /></main>

  const nomApprenant = apprenant ? `${apprenant.prenom} ${apprenant.nom}` : 'Apprenant NOVI'
    ...
    <main className="min-h-screen bg-gray-100 py-12 px-4">
      <div className="max-w-4xl mx-auto mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button onClick={telechargerPDF} className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-blue-700">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            Télécharger PDF
          </button>
          <Link href="/dashboard" className="border border-gray-200 text-gray-600 px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50">
            Tableau de bord
          </Link>
        </div>
      </div>

      <div ref={certificatRef} className="max-w-4xl mx-auto bg-white rounded-2xl overflow-hidden shadow-lg" style={{ aspectRatio: '297/210' }}>
        ...
        <div className="px-16 py-10 h-full flex flex-col">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900">NOVI Académie</h1>
          ...
          <div className="text-center flex-1 flex flex-col justify-center">
            ...
            <p className="text-4xl font-bold text-gray-900 mb-3">{nomApprenant}</p>
            ...
            <p className="text-gray-400 text-sm mb-8">{cours.duree_estimee} de formation</p>

            <div className="flex items-center justify-center gap-8 mb-8">
              <div className="text-center">
                <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-2">
                  <span className="text-2xl font-bold text-blue-600">{cours.score_qualite}</span>
                </div>
              ...
              <div className="text-center">
                <div className="w-16 h-16 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-2">
                  <span className="text-sm font-bold text-amber-600">Bien</span>
                </div>
      ...
      <div className="max-w-4xl mx-auto mt-6 bg-blue-50 rounded-2xl p-6 text-center">
        <p className="text-blue-900 font-semibold mb-1">Félicitations {nomApprenant} !</p>
        <p className="text-blue-600 text-sm">Ton certificat est valide 2 ans. Ajoute-le à ton profil Opportunity.</p>
        <Link href="/opportunity/profil/creer" className="inline-block mt-4 bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-medium hover:bg-blue-700">
