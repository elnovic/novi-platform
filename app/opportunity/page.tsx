      ...
      <div className="bg-gray-950 text-white px-8 py-16">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-4 gap-6 mt-10">
            {[
              { label: "Talents", valeur: "1 200+" },
              { label: "Entreprises", valeur: "85+" },
              { label: "Opportunités", valeur: "340+" },
        ...
        {chargement ? (
          ...
          <>
            ...
            <section className="mb-14">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900">
                  ...
                  <span className="text-sm font-normal text-gray-400 ml-2">({opportunites.length})</span>
                </h2>
              </div>

              {opportunites.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 rounded-2xl">
                  <p className="text-gray-400">Aucune opportunité disponible pour le moment.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  {opportunites.map(op => (
                    <div key={op.id} className="border border-gray-100 rounded-2xl p-6 hover:border-blue-200 transition-colors">
                      ...
                      <button className="w-full text-sm text-blue-600 border border-blue-200 py-2 rounded-lg hover:bg-blue-50 transition-colors">
                        Postuler →
                      </button>
                    </div>
                  ))}
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900">
                  Talents certifiés NOVI
                  <span className="text-sm font-normal text-gray-400 ml-2">({profils.length})</span>
                </h2>
                <Link href="/opportunity/profil/creer" className="text-sm text-blue-600 hover:underline">
                  Créer mon profil →
                </Link>
              </div>
              ...
              {profils.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 rounded-2xl border border-gray-100">
                  <p className="text-gray-500 mb-4">Aucun profil disponible pour le moment.</p>
                  <Link
                    href="/opportunity/profil/creer"
                  ...
                  >
                    Être le premier à créer un profil →
                  </Link>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  {profils.map(p => (
                    <div key={p.id} className="border border-gray-100 rounded-2xl p-6 hover:border-blue-200 transition-colors">
                      ...
                      {p.domaines && p.domaines.length > 0 && (
                      ...
                      {p.objectifs && p.objectifs.length > 0 && (
