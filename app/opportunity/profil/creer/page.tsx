...
import { creerProfilOpportunity, getUtilisateurConnecte } from '@/lib/supabase'
...
export default function CreerProfil() {
  ...
  const languesListe = ["Français", "Anglais", "Arabe", "Espagnol", "Portugais", "Allemand"]
  ...
  const validerEtape1 = () => {
    if (!form.titre || !form.ville) { setErreur('Remplis tous les champs obligatoires.'); return }
    setErreur(''); setEtape(2)
  }

  const validerEtape2 = () => {
    if (form.domaines.length === 0 || form.competences.length === 0) { setErreur('Ajoute au moins un domaine et une compétence.'); return }
    setErreur(''); setEtape(3)
  }

  const soumettre = async () => {
    if (form.objectifs.length === 0) { setErreur('Choisis au moins un objectif.'); return }
    setErreur(''); setChargement(true)
    try {
      const user = await getUtilisateurConnecte()
      await creerProfilOpportunity({
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-8 w-full max-w-lg">
          ...
          <div className="flex items-center gap-2 mb-8">
            {[1, 2, 3].map(n => (
              <div key={n} className="flex items-center gap-2 flex-1">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${n < etape ? 'bg-green-500 text-white' : n === etape ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
                  {n < etape ? '✓' : n}
                </div>
        ...
        {etape === 1 && (
          <div className="space-y-4">
            ...
            <button onClick={validerEtape1} className="w-full bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 text-sm">Continuer →</button>
          </div>
        )}
        ...
        {etape === 2 && (
          <div className="space-y-5">
              ...
              <div className="flex flex-wrap gap-2">
                {domainesListe.map(d => (
                  <button key={d} onClick={() => toggleItem(form.domaines, d, 'domaines')}
                    className={`text-xs px-3 py-1.5 rounded-full border transition-all ${form.domaines.includes(d) ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}>
                    {d}
                  </button>
                ))}
              </div>
            ...
            <div>
              <p className="text-xs font-medium text-gray-600 mb-2">Compétences *</p>
              <div className="flex gap-2 mb-2">
                ...
                <button onClick={ajouterCompetence} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 flex-shrink-0">+</button>
              </div>
              <div className="flex flex-wrap gap-2">
                {form.competences.map(c => (
                  <span key={c} className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-full flex items-center gap-1">
                    {c}
                    <button onClick={() => setForm(p => ({ ...p, competences: p.competences.filter(x => x !== c) }))} className="text-blue-400 hover:text-blue-700">×</button>
                  </span>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {languesListe.map(l => (
                  <button key={l} onClick={() => toggleItem(form.langues, l, 'langues')}
                    className={`text-xs px-3 py-1.5 rounded-full border transition-all ${form.langues.includes(l) ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}>
                    {l}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setEtape(1)} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 text-sm hover:bg-gray-50">← Retour</button>
              <button onClick={validerEtape2} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 text-sm">Continuer →</button>
            </div>
          </div>
        )}
        ...
        {etape === 3 && (
          <div className="space-y-5">
              ...
              <div className="flex flex-wrap gap-2">
                {objectifsListe.map(o => (
                  <button key={o} onClick={() => toggleItem(form.objectifs, o, 'objectifs')}
                    className={`text-xs px-3 py-1.5 rounded-full border transition-all ${form.objectifs.includes(o) ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}>
                    {o}
                  </button>
                ))}
              </div>
            ...
            {erreur && <p className="text-xs text-red-500">{erreur}</p>}
            <div className="flex gap-3">
              <button onClick={() => setEtape(2)} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 text-sm hover:bg-gray-50">← Retour</button>
              <button onClick={soumettre} disabled={chargement} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 text-sm disabled:opacity-60">
        ...
        {etape === 4 && (
          <div className="text-center space-y-6 py-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-2">Ton profil est maintenant visible sur Novi Opportunity.</p>
            </div>
            <Link href="/opportunity" className="block w-full bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 text-sm">
