...
export default function Dashboard() {
  ...
  const [onglet, setOnglet] = useState<'parcours' | 'certifications' | 'profil'>('parcours')
    ...
    <main className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 text-sm text-gray-600">
          <Link href="/academie" className="hover:text-blue-600">Académie</Link>
          <Link href="/opportunity" className="hover:text-blue-600">Opportunity</Link>
          <Link href="/research" className="hover:text-blue-600">Research</Link>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center">
            <span className="text-white text-xs font-bold">{apprenant.prenom?.[0]}{apprenant.nom?.[0]}</span>
          </div>
          <span className="text-sm text-gray-700 font-medium">{apprenant.prenom} {apprenant.nom}</span>
          <button onClick={seDeconnecter} className="text-xs text-gray-400 hover:text-red-500">Déconnexion</button>
        </div>
      </nav>

      <div className="max-w-6xl mx-auto px-8 py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Bonjour, {apprenant.prenom} 👋</h1>
            ...
            <div className="space-y-4">
              {coursEnCours.map(c => (
                <div key={c.id} className="flex items-center justify-between gap-6">
                  ...
                  <Link href={`/academie/${c.cours_id}/chapitre/${c.chapitre_actuel}`}
        ...
        {onglet === 'parcours' && (
          <div className="bg-white border border-gray-100 rounded-2xl p-6">
            <h2 className="font-semibold text-gray-900 mb-4">Cours recommandés</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { id: "introduction_a_lintelligence_artificielle", titre: "Introduction à l'IA", niveau: "Débutant", duree: "20h", domaine: "IA" },
                { id: "introduction_a_python_pour_la_data_sci", titre: "Python pour la Data Science", niveau: "Débutant", duree: "20h", domaine: "Programmation" },
                { id: "introduction_a_liot_pour_ingenieurs_deb", titre: "Introduction à l'IOT", niveau: "Débutant", duree: "20h", domaine: "IOT" }
              ].map(c => (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-blue-600 rounded-full flex items-center justify-center">
                  <span className="text-white text-xl font-bold">{apprenant.prenom?.[0]}{apprenant.nom?.[0]}</span>
                </div>
                <div>
              ...
              <Link href="/opportunity/profil/creer"
                className="inline-block mt-2 text-sm text-blue-600 border border-blue-200 px-4 py-2 rounded-xl hover:bg-blue-50">
